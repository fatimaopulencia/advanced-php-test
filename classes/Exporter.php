...
class Exporter {
    ...
    function getWhereStatement($search){
    ...
    function getPlayerStats($search) {
        ...
        $where = $this->getWhereStatement($search);
        $where = implode(' AND ', $where);
        $sql = "
                SELECT roster.name
                , player_totals.* 
                , (player_totals.3pt * 3) + (player_totals.2pt * 2) + player_totals.free_throws as 'total_points'
                , CASE WHEN player_totals.field_goals_attempted IS NOT NULL
                    THEN CONCAT(ROUND((player_totals.field_goals) / (player_totals.field_goals_attempted)*100,2),'%')
                    ELSE 0
                END AS 'fields_goals_acct'
                
                , CASE WHEN player_totals.3pt_attempted IS NOT NULL
                    THEN CONCAT(ROUND((player_totals.3pt) / (player_totals.3pt_attempted)*100,2),'%')
                    ELSE 0
                END AS '3pt_pct'
                
                , CASE WHEN player_totals.3pt_attempted IS NOT NULL
                    THEN CONCAT(ROUND((player_totals.2pt) / (player_totals.2pt_attempted)*100,2),'%')
                    ELSE 0
                END AS '2pt_pct'
                
                , CASE WHEN player_totals.free_throws_attempted IS NOT NULL
                    THEN CONCAT(ROUND((player_totals.free_throws) / (player_totals.free_throws_attempted)*100,2),'%')
                    ELSE 0
                END AS 'free_throws_pct'
                
                , player_totals.offensive_rebounds + player_totals.defensive_rebounds AS 'total_rebounds'
        ...
        $final_data = collect($data)->map(function ($array) {
        ...
        // $final_data = $collection->forget('players_total.player_id');
